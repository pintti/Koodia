    ...
    while city != library['end_city']:
        ...
        for next_city, next_road in cities[str(future[-1])]:
            if next_city != future[0] and next_city not in road_traveled:
                for next_city_2, next_road_2 in cities[str(future)[-1]]:
                    if next_city_2 != future[0] and next_city_2 not in road_traveled:
                        next_roads.append([next_city, next_city_2, next_road, next_road_2])
