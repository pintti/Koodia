...
def find_road(cities):
    ...
    for next_city, next_road in cities[str(city)]:
        ...
        start.append(next_city)
        traveled.append(next_city)
    while start:
        city = start.pop(-1)
        old_road = 0
        while city != library['end_city']:
            ...
            for next_city, next_road in cities[str(future[-1])]:
                if next_city not in future and next_city not in traveled:
            ...
            city = future[-1]
            print(roads)
        choices.append([future, old_road])
        print(choices)
    print(find_small(choices)[1])
    input()
    print(time.time() - now)
