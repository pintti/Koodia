...
class Player():
    ...
    def check_diag_vision(self, matrix, dx, dy, px, py):
        '''Check whether or not the player can see diagonally.'''
        diag_visible = []
        walls = 0
        try:
            if matrix[dy][px] == 1:
    ...
    def pick_up_pickaxe(self, matrix):
        '''Function adds pickaxe to players inventory.'''
...
def check_y_axis(matrix, y, x, path):
    ...
    next_y = [y]
    number = 0
    while next_y != [] and number < points['torch']:
...
def check_x_axis(matrix, y, x, path):
    ...
    next_x = [x]
    number = 0
    while next_x != [] and number < points['torch']:
